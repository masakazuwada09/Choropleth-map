...
const ImaginTestFormModal = (props, ref) => {
	...
	const show = (showData = null) => {
		...
		setTimeout(() => {
			...
			if (showData?.id) {
				...
				setValue("img_rate", showData?.img_rate);
			}
		}, 300);
		if (showData?.id) {
			setTest(showData);
	...
	const submit = (data) => {
		...
		formData.append("img_rate", data?.img_rate);
		formData.append("type", data?.type);

		let url = `v1/laboratory/imaging/store`;
		if (test?.id) {
			url = `v1/laboratory/imaging/update/${test?.id}`;
...
export default forwardRef(ImaginTestFormModal);
